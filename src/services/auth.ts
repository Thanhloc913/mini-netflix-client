...
export async function login(payload: LoginRequest): Promise<TokenPair> {
  try {
    const response = await apiClient.post<TokenPair>("/login", payload);
...
export async function refreshToken(refresh_token: string): Promise<TokenPair> {
  try {
    const response = await apiClient.post<TokenPair>("/refresh", { refresh_token });
    return response.data;
  } catch (error: any) {
...
export async function register(payload: RegisterRequest, avatar?: File): Promise<RegisterResponse> {
  ...
  return postFormData<RegisterResponse>("/register", formData);
}
...
export async function getProfileById(id: string): Promise<Profile> {
  try {
    const response = await apiClient.get<Profile>(`/profile/account/${id}`);
    return response.data;
  } catch (error: any) {
...
export async function updateProfile(id: string, name: string, avatar?: File): Promise<Profile> {
  try {
    ...
    const response = await apiClient.patch(`/profile/${id}`, formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    });
...
export async function deleteProfile(id: string): Promise<{ message: string }> {
  try {
    const response = await apiClient.delete<{ message: string }>(`/profile/${id}`);
    return response.data;
  } catch (error: any) {
...
export async function getProfileByAccountId(accountId: string): Promise<Profile> {
  try {
    ...
    const response = await apiClient.get<Profile>(`/profile/account/${accountId}`);
...
export async function changePassword(currentPassword: string, newPassword: string): Promise<{ message: string }> {
  try {
    const response = await apiClient.post<{ message: string }>("/change-password", {
      currentPassword,
      newPassword
    });
    return response.data;
