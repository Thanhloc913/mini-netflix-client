    ...
    if (response.data.access_token) {
      try {
        ...
        if (tokenPayload.role === "ADMIN") {
          throw new AuthError(
            "Tài khoản Admin không được phép đăng nhập ở đây",
            403
          );
        }
      } catch (jwtError) {
...
export async function getProfiles(): Promise<Profile[]> {
  try {
    ...
    const message =
      error.response?.data?.message ||
      error.message ||
      "Lấy danh sách profile thất bại";
    throw new AuthError(message, error.response?.status);
  }
}
...
export async function getProfileByAccountId(
  ...
  try {
    ...
    const response = await apiClient.get<Profile>(
      `/auth/profile/account/${accountId}`
    );
    ...
    const message =
      error.response?.data?.message || error.message || "Lấy profile thất bại";
    throw new AuthError(message, error.response?.status);
  }
}
...
export async function deleteProfile(id: string): Promise<{ message: string }> {
  try {
    ...
    const message =
      error.response?.data?.message || error.message || "Xóa profile thất bại";
    throw new AuthError(message, error.response?.status);
  }
}
